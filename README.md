...
- [ x] Be able to use a `say` command to say things that other people in the room will see (server implementation incomplete)
- [ x] Upon login, subscribe to a Pusher channel based on the player's universally unique id: `p-channel-<uuid>`
- [x ] Bind the player channel to `broadcast` events and display the messages to the player
- [ x] Alert the player when someone enters and leaves the current room (implemented on server)
- [x ] Alert the player when someone in the current room says something (server implementation incomplete)
...
- [x ] Add a `shout` command that broadcasts a message to every player
...
`curl -X POST -H "Content-Type: application/json" -d '{"username":"testuser", "password1":"testpassword", "password2":"testpassword"}' https://tomprojectweekmudserver.herokuapp.com/api/registration/`
* Response:
...
* Request:
  * `curl -X POST -H "Content-Type: application/json" -d '{"username":"testuser", "password":"testpassword"}' localhost:8000/api/login/`
  ...
  `curl -X POST -H "Content-Type: application/json" -d '{"username":"testuser", "password":"testpassword"}' https://tomprojectweekmudserver.herokuapp.com/api/login/`
* Response:
  * `{"key":"6b7b9d0f33bd76e75b0a52433f268d3037e42e66"}`
...
`curl -X GET -H 'Authorization: Token dce04385150b8311b225c111b2a35c45ac5dc46c' https://tomprojectweekmudserver.herokuapp.com/api/adv/init/`
* Response:
...
### Move
* Request:  (Replace token string with logged in user's auth token)
  * `curl -X POST -H 'Authorization: Token 6ee58d31645e8e1e7ce5eea9cab836a205a3d637' -H "Content-Type: application/json" -d '{"direction":"n"}' localhost:8000/api/adv/move/`
* Response:
